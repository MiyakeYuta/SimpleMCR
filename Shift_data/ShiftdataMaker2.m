...
rng("shuffle");
center_pos = randi([1, sig_size], 1, pk_num);
...
%             tst_pk(j) = tst_pk(j) + pk_amp(i)  / (1 + ((j - center_pos(i)) / (0.5 * pk_sigma(i)))^2);
            ...
            tst_pk(j) = tst_pk(j) + pk_amp(i) * exp(-(x(j) - center_pos(i))^2 / (2 * pk_sigma(i)^2));
        end
    end
  
    tst_sig_seq(:,l) = tst_pk';
    if noise_add == "on"
        noise = noise_amp * rand(sig_size, 1);
